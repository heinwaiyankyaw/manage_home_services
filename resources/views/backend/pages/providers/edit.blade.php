        ...
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing mt-5">
            <div class="widget-content widget-content-area br-6">
                <div class="d-flex justify-content-center align-items-center mb-4">
                    <a href="{{ route('admin.providers.list') }}" class="btn btn-sm btn-outline-primary" title="Go back">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
                        </svg> Back
                    </a>
                    <h4 class="mb-0 text-center flex-grow-1">Edit Provider</h4>
                ...
                <form action="{{ route('admin.providers.update', $provider->id) }}" method="POST">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="text" class="form-control" id="name" name="name"
                            value="{{ $provider->name }}" placeholder="Enter Provider name">
                        @error('name')
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="email" class="form-control" id="email" name="email"
                            value="{{ $provider->email }}" placeholder="Enter Provider email">
                        @error('email')
                            <div class="text-danger mt-2">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-4">
                        ...
                        <select class="form-control" id="status" name="status">
                            ...
                            <option value="inactive" {{ $provider->status == 'inactive' ? 'selected' : '' }}">Inactive
                            </option>
                        </select>
                        @error('status')
                            <div class="text-danger mt-2">{{ $message }}</div>
                    ...
                    <div class="form-check form-switch mb-4">
                        ...
                        <label class="form-check-label" for="change_password_toggle">Change Password</label>
                    </div>
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-block py-3" title="Update Provider">
                            <i class="fas fa-plus-circle mr-2"></i> Update Provider
                        </button>
                    </div>
                </form>
            </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const switchToggle = document.getElementById('change_password_toggle');
